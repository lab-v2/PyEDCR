...
class EDCR_LTN_experiment(EDCR):
    def __init__(self,
        ...
        self.scheduler_step_size = self.num_ltn_epochs
        ...
        self.datapreprocessing,_, self.loaders, self.devices = (
        ...
        with context_handlers.TimeWrapper():
            ...
            for batch_num, batch in enumerate(loader):
                ...
                with context_handlers.ClearCache(device=device):
                    X, Y_fine_grain, Y_coarse_grain, indices = (batch[0].to(device), batch[1].to(device), batch[3].to(device), batch[4])
                    ...
                    if mode == 'train' and optimizer is not None and scheduler is not None:
                        ...
                        if loss == 'BCE':
                            ...
                            #print('NN loss:', criterion(Y_pred, Y_combine).item())
                            #print('LTN Loss:', (1. - sat_agg).item())
                            print('params:', sigma1.item(),sigma2.item())
                            #print(len(batch))
                            batch_total_loss = (0.5 / (sigma1)**2) * (1. - sat_agg) + \
                                              (0.5 / (sigma1)**2) * (criterion(Y_pred, Y_combine)) - \
                                              (1/sigma1 +1/sigma2)
                            
                            print(batch_total_loss.item())
                        ...
                        if loss == "soft_marginal":
                            ...
                            batch_total_loss =  beta_parameter*(1. - sat_agg) + (criterion(Y_pred, Y_combine))
    ...
    def train_and_evaluate_baseline_combined_model(self,
        ...
        with context_handlers.TimeWrapper():
            ...
            #batches = neural_fine_tuning.get_fine_tuning_batches(train_loader=loader,
                                                                 #num_batches=num_batches,
                                                                 #debug=False)
            batches = tqdm(enumerate(loader, 0),total=num_batches)
    ...
    def run_learning_pipeline(self,
        ...
        '''
        if beta_learnable:
           beta_param = nn.Parameter(torch.clamp(torch.randn(1).to(self.devices[0]),1,100))
           model_parameters = list(self.correction_model[model_index].parameters()) + [beta_param]
        ...
        '''
        sigma1 = nn.Parameter(torch.randn(1).to(self.devices[0]))
        sigma2 = nn.Parameter(torch.randn(1).to(self.devices[0]))
        model_parameters = list(self.correction_model[model_index].parameters()) + [sigma1,sigma2]
        optimizer = torch.optim.Adam(params=model_parameters,
                                     lr=config.ltn_lr)
