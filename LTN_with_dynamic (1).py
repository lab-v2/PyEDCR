    ...
    def fine_tune_and_evaluate_combined_model(self,
        ...
        if self.beta_learn is True:
             ...
             optimizer = torch.optim.AdamW([
                {'params': fine_tuner.parameters(), 'lr': self.lr},  # Default learning rate for most parameters
                {'params': [sigma_1,sigma_2], 'lr': 5e-5}  # Additional parameter with different learning rate
            ], weight_decay = 0.01)
             scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer,
                                                    step_size= config.scheduler_step_size,
                                                    gamma=self.scheduler_gamma)
            ...
            shape = (1,)
            temperature = nn.Parameter(torch.Tensor([1.0]).to(device),requires_grad = True)
            
            optimizer = torch.optim.AdamW([
                {'params': fine_tuner.parameters(), 'lr': self.lr},  # Default learning rate for most parameters
                {'params': temperature, 'lr': 5e-5}  # Additional parameter with different learning rate
            ], weight_decay = 0.01)
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer,
                                                        step_size=config.scheduler_step_size,
                                                        gamma=self.scheduler_gamma)
            
            #scheduler_temp = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
                                   #torch.optim.Adam([{'params': optimizer.param_groups[1]['params'], 
                                   #'lr': 1e-5}]),T_0 = 5)
        ...
        for epoch in range(self.num_ltn_epochs):
            ...
            with ((context_handlers.TimeWrapper())):
                ...
                for batch_num, batch in batches:
                    with context_handlers.ClearCache(device=device):
                        ...
                        if self.dynamic_weight_averaging is True:
                            ...
                            print('LTN_loss',len(LTN_losses))
                            print('NN_loss',len(NN_losses))
                            ...
                            if len(ratios_LTN) < 1 and len(ratios_NN) < 1:
                               ...
                               alpha_2 = torch.randn(1)
                                ...
                                ratio_tensor = torch.Tensor([ratio_LTN,ratio_NN]).to(device)

                                softmax = torch.nn.Softmax(dim = -1)
                                alphas = softmax(ratio_tensor/temperature).to(device)
                                alpha_1 = 2*alphas[0]
                                alpha_2 = 2*alphas[1]
                            ...
                            temperature.data = torch.clamp(temperature.data, min=1e-3,max = 1e3)
                            print('temperature:', temperature.item())
                            print('alpha_1:', alphas[0].item())
                            print('alpha_2:', alphas[1].item())





                        elif self.beta_learn is True:
                ...
                ## updating the learning rate after an epoch 
                scheduler.step()
