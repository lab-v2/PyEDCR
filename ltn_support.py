...
def compute_sat_normally(logits_to_predicate,
    ...
    fine_label_dict = {name: label for label, name in enumerate(data_preprocessing._fine_grain_classes_str)}
    coarse_label_dict = {name: label + len(data_preprocessing._fine_grain_classes_str) for label, name in
                         enumerate(data_preprocessing._coarse_grain_classes_str)}
    labels_fine = labels_fine.detach().to('cpu')
    labels_coarse = labels_coarse.detach().to('cpu') + len(data_preprocessing._fine_grain_classes_str)

    # Define constant
    l = {}
    num_labels = len(data_preprocessing._fine_grain_classes_str) + len(data_preprocessing._coarse_grain_classes_str)
...
def compute_sat_testing_value(logits_to_predicate,
    ...
    fine_label_dict = {name: label for label, name in enumerate(data_preprocessing._fine_grain_classes_str)}
    coarse_label_dict = {name: label + len(data_preprocessing._fine_grain_classes_str) for label, name in enumerate(data_preprocessing._coarse_grain_classes_str)}
    labels_fine = labels_fine.detach().to('cpu')
    labels_coarse = labels_coarse.detach().to('cpu') + len(data_preprocessing._fine_grain_classes_str)
